        ...
        try {
            while (reader.hasNext()) {
                if (reader.next() == XMLStreamConstants.START_ELEMENT
                    ...
                    if (!SpreadsheetManipulator.attribute(reader, "Type").toLowerCase(Locale.ROOT).endsWith("worksheet"))
                        continue;

                    String id = SpreadsheetManipulator.attribute(reader, "Id");
                    String target = SpreadsheetManipulator.normalizeTarget(SpreadsheetManipulator.attribute(reader, "Target"));
